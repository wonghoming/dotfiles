...
class GitBlame {
    ...
    copyHash() {
        return __awaiter(this, void 0, void 0, function* () {
            const commitInfo = yield this.getCommitInfo();
            try {
                yield vscode_1.env.clipboard.writeText(commitInfo.hash);
                vscode_1.window.showInformationMessage("Copied hash to clipboard");
            }
            catch (err) {
                errorhandler_1.ErrorHandler.logCritical(err, `Unable to copy hash to clipboard. hash: ${commitInfo.hash}`);
            }
        });
    }
    copyToolUrl() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (commitToolUrl) {
                try {
                    yield vscode_1.env.clipboard.writeText(commitToolUrl.toString());
                    vscode_1.window.showInformationMessage("Copied tool URL to clipboard");
                }
                catch (err) {
                    errorhandler_1.ErrorHandler.logCritical(err, `Unable to copy tool URL to clipboard. URL: ${commitToolUrl}`);
                }
            }
            else {
                vscode_1.window.showErrorMessage("Missing gitblame.commitUrl configuration value.");
